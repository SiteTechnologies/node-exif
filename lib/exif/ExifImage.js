...
ExifImage.prototype.extractExifData = function (data, start, length) {
  ...
  const markerNoteTag = ExifImage.TAGS.exif[0x927C];
  if (typeof exifData.exif[markerNoteTag] != "undefined") {

    // Check the header to see what kind of Makernote we are dealing with
    if (typeof exifData.exif[markerNoteTag] === 'string') {
      exifData.makernote = exifData.exif[markerNoteTag];
    } else if (typeof exifData.exif[markerNoteTag] === 'Buffer') {
      let type;

      if (exifData.exif[markerNoteTag].getString(0, 7) === "OLYMP\x00\x01" || exifData.exif[markerNoteTag].getString(0, 7) === "OLYMP\x00\x02") {
        type="olympus"

      } else if (exifData.exif[markerNoteTag].getString(0, 7) === "AGFA \x00\x01") {
        type="agfa";

      } else if (exifData.exif[markerNoteTag].getString(0, 8) === "EPSON\x00\x01\x00") {
        type="epson";

      } else if (exifData.exif[markerNoteTag].getString(0, 8) === "FUJIFILM") {
        type="fujifilm";

      } else if (exifData.exif[markerNoteTag].getString(0, 9) === "Panasonic") {
        type="panasonic";

      } else if (exifData.exif[markerNoteTag].getString(0, 5) === "SANYO") {
        type="sanyo";
      }
